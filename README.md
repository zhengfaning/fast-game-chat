...
- 🧪 **压力测试验证**: 已通过单机 **100,000+** 并发用户的严苛测试，**100% 成功率**，平均响应延迟仅为 **0.67ms**。
- ⚡ **高性能架构**: 采用 **RobustMQ (MQTT)** 消息队列解耦逻辑、并发消息消费以及异步持久化（Write-Behind）技术，完美消除 I/O 瓶颈。
- 🔌 **消息队列**: 默认使用 **RobustMQ**，提供微秒级延迟、消息持久化和多实例支持。同时保留 Redis Pub/Sub 兼容性，可通过配置切换。

## 🚀 性能表现 (Performance)

系统在单机环境下（AMD Ryzen 7 7840HS, 16GB内存）通过了极限压力测试，核心指标如下：

| 指标 | 测试数据 | 说明 |
| :--- | :--- | :--- |
| **并发连接数** | **100,000** | 稳定维持十万级 WebSocket 长连接 |
| **消息成功率** | **100.00%** | 在极端建立连接风暴下无一丢包 |
| **平均延迟** | **0.67 ms** | 使用 RobustMQ 后延迟降低 70%+ |
| **最大延迟** | **7.64 ms** | 即使在峰值波动下也能保持极快响应 |
| **消息吞吐量** | **850+ req/s** | RobustMQ 提供更高的吞吐性能 |

详细测试报告请参阅：[100000用户高并发测试报告](docs/STRESS_TEST_100000_USERS_REPORT.md)
...
客户端 A (MMO) ──┐
                 │    连接 (WS)    ┌──────────┐    发布 (Pub)    ┌──────────┐    消费 (Sub)    ┌──────────┐
客户端 B (Card) ─┼───────────────→ │ 统一网关 │  ────────────→  │RobustMQ  │  ────────────→  │ 聊天服务 │
                 │                 │ Gateway  │  ←────────────  │  (MQTT)  │  ←────────────  │ Chat Svc │
...
| `make docker-up` | 启动 Docker 基础服务 (PostgreSQL, Redis, RobustMQ) |
